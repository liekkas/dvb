...
object CalcKpiByHour {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val conf = new SparkConf().setAppName("LiveByHour")
...
//    od.persist(StorageLevel.MEMORY_AND_DISK_SER)
    ...
    val channelTypeResult = sqlContext.sql(
      s"""
        select day,channel_type,
               ...
               channel_user_num *1.0/cover_user_num as cover_pct,
    ...
    val groupByChannel = sqlContext.sql(
      """
        select day as channel_day,
          ...
          from origin_data
         where channel_type="央视频道" or channel_type="卫视频道"
         group by day,hour,channel_type,channel_name
      """.stripMargin
    )
    ...
    val channelJoined = sqlContext.sql(
      """
        select b.day,b.hour,a.channel_type,a.channel_name,a.channel_user_num,a.channel_time_in_use,
               b.user_num,b.time_in_use,b.cover_user_num
