...
/**
  ...
  */
object CalcKpiByMonth {
  case class ORIGIN_DATA(uid:String,month:Int,time_in_use:Long,channel_name:String)
  ...
  def main(args: Array[String]): Unit = {
    ...
    val conf = new SparkConf().setAppName("CalcKpiByMonth")
    ...
    val data = sc.textFile(args(0))
      .filter(_ != "")
      .map(_.split("\t"))
      .filter(p => (p(1) >= args(4) && p(1) <= args(5))) //统计指定时间范围内的数据
      .map(p => ORIGIN_DATA(p(0),p(1).toInt,p(7).toLong,p(5))).toDF()
    ...
    val od = data.join(channelDict, data("channel_name")===channelDict("dict_key"), "left")
      .select("uid","month","time_in_use","channel_name","channel_type")
    ...
    val groupByChannel = sqlContext.sql(
      """
        select month as channel_month,
          ...
          from origin_data
         where channel_type="央视频道" or channel_type="卫视频道"
         group by month,channel_type,channel_name
      """.stripMargin
    )
    ...
    val channelJoined = sqlContext.sql(
      """
        select b.month,a.channel_type,a.channel_name,a.channel_user_num,a.channel_time_in_use,
               b.user_num,b.time_in_use,b.cover_user_num
    ...
    val channelResult = sqlContext.sql(
      s"""
        select month,channel_name,channel_type,
               channel_time_in_use/${FIX_TIME}/cover_user_num*${USER_INDEX_OFFSET} as user_index,
               channel_time_in_use/time_in_use as market_pct,
               channel_user_num*1.0/user_num as cover_pct,
               channel_time_in_use/channel_user_num as time_in_use_avg,
