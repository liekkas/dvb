...
/**
  ...
  */
object DemandByHour {
  case class DEMAND_DATA(uid:String,day:String,hour:Int,time_in_use:Long,show_name:String)
  ...
  val USER_INDEX_OFFSET = 1000 //用户指数权重指数
  ...
  def main(args: Array[String]): Unit = {
    ...
    //加入节目类型 - 当做原始表看待
    val demandData = data.join(showDict, data("show_name")===showDict("dict_key"), "left")
      .select("uid","day","hour","time_in_use","show_name","show_type")
    ...
    val summary = hourSum.join(uidCount, hourSum("day")===uidCount("date"), "left")
      .select("day","hour","user_num","cover_user_num","time_in_use","request_times","all_show_num")
    ...
    val showTypeJoined = sqlContext.sql(
      """
        select b.day,b.hour,a.show_type,a.show_user_num,a.show_time_in_use,show_num,
