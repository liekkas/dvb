...
object DemandByWeek {
  ...
  def main(args: Array[String]): Unit = {
    val summaryWFile = new File(args(2) + File.separator + "T_USER_SUMMARY_W")
    val demandWFile = new File(args(2) + File.separator + "T_DEMAND_BROADCAST_W")
    val demandShowsWFile = new File(args(2) + File.separator + "T_DEMAND_BROADCAST_SHOWS_W")
...
//    val jedis = new Jedis("localhost")
    ...
    val data = sc.textFile(args(0))
      ...
      .map(p => DEMAND_DATA(p(0),p(9).toInt,p(3).toLong,p(6))).toDF().cache()
...
//          jedis.sadd("DEMAND_W", demandMovie)
          FileUtils.writeStringToFile(demandWFile,demandMovie,true)
