...
/**
  ...
  */
object LiveByMonth {
  case class LIVE_DATA(uid:String,month:Int,remain_time:Long,channel_name:String)
  ...
  def main(args: Array[String]): Unit = {
    ...
    val data = sc.textFile(args(0))
      .map(_.split("	")).map(p => LIVE_DATA(p(0),p(10).toInt,p(3).toLong,p(6))).toDF()
    ...
    sqlContext.sql(
      """
        select month,channel_name,count(distinct uid) as usernum,sum(remain_time)/60 remain_time,count(*) request_times
    ...
    months.foreach(month => {
      ...
      if(size > 0 && lastSize > 0) {
        ...
        //电影类
        val movie = sqlContext.sql("select count(distinct(t.channel_name)) channel_name_num, sum(t.remain_time) remain_time, sum(t.request_times) request_times, sum(t.usernum) usernum " +
          "from demand_channel t where t.month=" + month + " and t.show_type='电影'").first()
        ...
        jedis.sadd("LIVE_M", demandM_movie)
        println(">>> Complete demandM_movie:"+demandM_movie)

        //电视剧类
        val tv = sqlContext.sql("select count(distinct(t.channel_name)) channel_name_num, sum(t.remain_time) remain_time, sum(t.request_times) request_times, sum(t.usernum) usernum " +
          "from demand_channel t where t.month=" + month + " and t.show_type='电视剧'").first()
        ...
        shows.foreach(show => {
          ...
          jedis.sadd("LIVE_SHOWS_M", demandShowsM)
